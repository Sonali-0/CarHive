...
export default function EditPost() {
  ...
  useEffect(() => {
    const fetchCarDetails = async () => {
      ...
      try {
        const response = await fetch(`https://carhivebackend.vercel.app/api/cars/${id}/`, {
          headers: {
  ...
  const handleDeleteImage = async (imageId) => {
    ...
    try {
      const response = await fetch(`https://carhivebackend.vercel.app/api/carimages/${imageId}/`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${accessToken}`,
        },
